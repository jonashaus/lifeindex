...
const CreateHabitForm = ({ setOpen }) => {
  const { createHabit } = useContext(LifeIndexContext);
  const form = useForm({
    resolver: zodResolver(habitFormSchema),
    defaultValues: { name: "", points: 10 },
    mode: "onChange",
  });
  ...
  const submitBeforeDialogClose = async (e) => {
    e.preventDefault();
    form.handleSubmit(
      async (data) => await createHabit(data).then(() => setOpen(false))
    )();
  };
